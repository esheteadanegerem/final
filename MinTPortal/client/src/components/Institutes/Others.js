...
const Others = () => {
  ...
  useEffect(() => {
    ...
    const fetchAgencies = async () => {
      try {
        const response = await axios.get('https://final-0t4v.onrender.com/institutes/others');
  ...
  function display(){
    ...
    if(loaded){
        for (let i = 0; i < agencies.length; i++) {
            ...
            if(agencies[i].category !=="Research Institutes" && agencies[i].category !=="Laboratories"
            && agencies[i].category!=="Ict Partners" && agencies[i].category!=="Government Agencies"){
                arr.push(
                    <div className="mb-5">
                    ...
                    />
                        ...
                        <div className="card-body">
                            ...
                            <a href={agencies[i].link} target="_blank" rel="noopener noreferrer">
                                {agencies[i].title}
                            </a>
                            </h4>
                            <p className="col card-text text-muted">{agencies[i].description}</p>
                            <h6 className="my-2"><b>Category:</b> {agencies[i].category}</h6>
                            <h6 className="my-2"><b>Email:</b> {agencies[i].email}</h6>
                            <h6 className="my-2"><b>Phone:</b> {agencies[i].phone}</h6>
                        </div>
                        </div>
                    </div>
                    </div>
  ...
  return (
    <div className="container m-10">
      <br /><br />
      <h1 className="mb-4 mt-3 font-weight-bold text-center">Other Partners</h1>
